...
class RpgCommands extends PluginBase implements CommandExecutor{
    ...
    public function onCommand(CommandSender $p, Command $cmd, $label, array $args) {
        if(\strtolower($cmd->getName() == "rpg")) {
            $mainlevel = $this->getOwner()->config->get("MainRPGWorld");
                    ...
                    switch(\strtolower($args[1])) {
                        case "mage":
                            if($this->getOwner()->hasClass($p)){
                                ...
                                $p->teleport($this->getOwner()->getServer()->getLevelByName($mainlevel)->getSafeSpawn());
                            }
                        return true;
                        ...
                        return true;
                            ...
                            /*case "archer":
                                if($this->getOwner()->hasClass($p)){
                                    ...
                                    $p->teleport($this->getOwner()->getServer()->getLevelByName($this->getOwner()->config->get("RPGworld", [])[0])->getSafeSpawn());
                                }
                            return true;*/
                    }
